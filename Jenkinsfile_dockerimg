                ...
                script {
                    sshagent(['aws-key']) {
                        withCredentials([usernamePassword(credentialsId: 'docker-hub', passwordVariable: 'dockerpwd', usernameVariable: 'dockeruser')]) {
                            ...
                            sh "ssh ${docker_develop_server} sudo docker image push ${IMAGE_NAME}:${BUILD_NUMBER}"
                ...
                script {
                    sshagent(['aws-key']) {
                        withCredentials([usernamePassword(credentialsId: 'docker-hub', passwordVariable: 'dockerpwd', usernameVariable: 'dockeruser')]) {
                            ...
                            sh "ssh ${docker_deploy_server} sudo docker container run -itd --rm -P ${IMAGE_NAME}:${BUILD_NUMBER}"
                        }
                    }
                }
            }
